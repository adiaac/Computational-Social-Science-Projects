...
```{r}
#define models
outcome_model <- as.formula(mortality ~ blood_pressure_medication + age + simplified_race + income_thousands)

propensity_model <- as.formula(blood_pressure_medication ~ age + sex_at_birth + simplified_race + income_thousands + college_educ + bmi + chol + blood_pressure)

tmle_result <- tmle(Y = heart_disease_no2$mortality,
                    A =heart_disease_no2$blood_pressure_medication,
                    W = model.matrix(outcome_model, heart_disease_no2),
